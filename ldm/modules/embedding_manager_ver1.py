...
class EmbeddingManager(nn.Module):
    def __init__(
        ...
        self.attention = TransformerBlock(dim=token_dim, n_heads=8, d_head=64, dropout = 0.1,dim_out=self.max_vectors_per_token*token_dim) # max_vectors_per_token*token_dim -> 10개의 토큰 한번에 계산
        if per_image_tokens:
    ...
    def forward(
        ...
        for placeholder_string, placeholder_token in self.string_to_token_dict.items(): # self.string_to_token_dict.items() = dict_items([('*', tensor(265))])
            if self.initial_embeddings is None:
                ...
                placeholder_embedding = self.attention(self.initial_embeddings.view(b,1,768).to(device), self.initial_embeddings.view(b,1,768).to(device))[-1].view(self.max_vectors_per_token,768) # self.attention을 통과시킨 [1, 7680]을 [10, 768]로 view. -> 여기서 self.attention을 거쳐서 임베딩이 반환됨. placeholder_embedding = self.attention 레이어 거친 스페셜 토큰 임베딩들
            ...
            if prospect_words is not None:
                if isinstance(prospect_words, list) and len(prospect_words)==self.max_vectors_per_token+1: # 이 두 조건을 만족시키지 않으면 위에 만들어뒀던 self.embedded_texts 리스트 그대로 쓰겠다.
